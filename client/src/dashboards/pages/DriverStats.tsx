...
import { CalendarIcon, Download, TrendingUp, UserCheck, Calendar, BarChart3, Activity, Sparkles } from "lucide-react"
    ...
    return (
      <div className="bg-white border border-gray-200 rounded-xl shadow-2xl p-4 backdrop-blur-sm">
        ...
        <div className="flex items-center gap-2">
          ...
          ></span>
          <p className="text-sm text-gray-600 font-medium">{`عدد السائقين: ${payload[0].value.toLocaleString()}`}</p>
        </div>
      </div>
    )
  }
...
export default function EnhancedDriverStats() {
  ...
  useEffect(() => {
    setLoading(true)
    dispatch(fetchChauffeurs({ search: searchQuery, limit: Limit })).finally(() => setLoading(false))
  ...
  const { chartData, totalDrivers, averagePerDay, peakDay } = useMemo(() => {
          ...
          <div className="flex items-center justify-between">
            <div className="space-y-2">
              ...
              <p className="text-lg text-gray-600 max-w-2xl">
                تتبع  بيانات تسجيل السائقين عبر الفترات الزمنية مع رؤى تحليلية متقدمة
              </p>
            </div>
            <Badge
              variant="secondary"
            ...
            >
              ...
              {chauffeurs.length.toLocaleString()} سائق
            </Badge>
          </div>
          ...
          <div className="grid gap-6 md:grid-cols-3">
            <Card className="border-0 shadow-xl bg-gradient-to-br from-blue-50 via-blue-100 to-blue-50 hover:shadow-2xl transition-all duration-500 hover:scale-105 group">
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-3">
                <CardTitle className="text-base font-semibold text-blue-800">إجمالي السائقين</CardTitle>
              ...
              <CardContent>
                <div className="text-3xl font-bold text-blue-900 mb-2">{totalDrivers.toLocaleString()}</div>
            ...
            <Card className="border-0 shadow-xl bg-gradient-to-br from-green-50 via-green-100 to-green-50 hover:shadow-2xl transition-all duration-500 hover:scale-105 group">
              ...
              <CardContent>
                ...
                <p className="text-sm text-green-600 font-medium">سائق يومياً</p>
              </CardContent>
            </Card>
              ...
              <Card className="border-0 shadow-2xl overflow-hidden">
                <CardHeader className="bg-gradient-to-r from-blue-50 via-purple-50 to-blue-50 border-b">
                  <CardTitle className="text-blue-800 text-2xl font-bold flex items-center gap-3">
                    ...
                    إحصائيات السائقين - عرض المساحة
                  </CardTitle>
                  <CardDescription className="text-blue-600 text-lg">
                    عرض تدفق تسجيل السائقين عبر الوقت مع تمييز الاتجاهات والأنماط
                  </CardDescription>
                </CardHeader>
                <CardContent className="pt-8">
                  <ResponsiveContainer width="100%" height={500}>
                    <AreaChart data={chartData} margin={{ top: 20, right: 30, left: 20, bottom: 80 }}>
                      <defs>
                        <linearGradient id="driverAreaGradient" x1="0" y1="0" x2="0" y2="1">
              ...
              <Card className="border-0 shadow-2xl overflow-hidden">
                <CardHeader className="bg-gradient-to-r from-green-50 via-blue-50 to-green-50 border-b">
                  <CardTitle className="text-green-800 text-2xl font-bold flex items-center gap-3">
                    ...
                    إحصائيات السائقين - الأعمدة
                  </CardTitle>
                  <CardDescription className="text-green-600 text-lg">
                    مقارنة أعداد السائقين المسجلين يومياً مع تمييز الفترات الأكثر نشاطاً
                  </CardDescription>
                </CardHeader>
                <CardContent className="pt-8">
                  <ResponsiveContainer width="100%" height={500}>
                    <BarChart data={chartData} margin={{ top: 20, right: 30, left: 20, bottom: 80 }}>
                      <defs>
                        <linearGradient id="driverBarGradient" x1="0" y1="0" x2="0" y2="1">
              ...
              <Card className="border-0 shadow-2xl overflow-hidden">
                <CardHeader className="bg-gradient-to-r from-purple-50 via-pink-50 to-purple-50 border-b">
                  <CardTitle className="text-purple-800 text-2xl font-bold flex items-center gap-3">
                    ...
                    إحصائيات السائقين - الخط البياني
                  </CardTitle>
                  <CardDescription className="text-purple-600 text-lg">
                    تتبع اتجاه تسجيل السائقين عبر الوقت مع إظهار الاتجاهات والتغيرات
                  </CardDescription>
                </CardHeader>
                <CardContent className="pt-8">
                  <ResponsiveContainer width="100%" height={500}>
                    <LineChart data={chartData} margin={{ top: 20, right: 30, left: 20, bottom: 80 }}>
                      <defs>
                        <linearGradient id="driverLineGradient" x1="0" y1="0" x2="1" y2="0">
